...
bool mySearch(int time){
	int cur_busNum = 0;
	for(int i = 0;i<n;){
		int j = i;
		while(j < n && j - i < c){
			if(arrived[j] - arrived[i] > time) break;
			++j;
		}
		++cur_busNum;
		i = j;
...
int main(int argc, char const *argv[])
{
	...
	cin>>n>>m>>c;
	for(int i = 0;i<n;++i) cin>>arrived[i];
	sort(arrived,arrived+n);
	{
		/*
		for(int i = 0;i<n;++i){
			for(int j = i;j<n;++j){
